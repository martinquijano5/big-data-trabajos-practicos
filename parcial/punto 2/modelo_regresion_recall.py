...
def run_logistic_regression_minimal(X_train, y_train, X_test, y_test, random_state=42):
    ...
    auc = roc_auc_score(y_test, y_pred_proba)
    ks = calculate_ks(y_test, y_pred_proba)
    # Calculate recall for the positive class (assuming 1)
    recall = recall_score(y_test, y_pred, pos_label=1, zero_division=0) # Added zero_division handling

    # Return model and metrics including recall
    return model, accuracy, auc, ks, recall
...
def sensitivity_analysis_logistic_regression(df, target_column, random_state=42):
    ...
    Identifies and returns the model with the highest RECALL (for class 1)
    during the process, along with the test data.
    ...
    Returns:
        tuple: (results_df, fig, best_model, best_model_features, X_test_data, y_test_data, best_removed_group)
            results_df (pd.DataFrame): DataFrame tracking removed groups, accuracy, AUC, KS, and Recall.
            fig (matplotlib.figure.Figure): Figure object for the accuracy plot.
            best_model (LogisticRegression): The trained model instance with the highest recall.
            ...
            best_removed_group (str): The name of the best removed group based on recall.
    """
    X = df.drop(target_column, axis=1)
    y = df[target_column]
    ...
    # Updated call to minimal runner to get recall
    initial_model, initial_accuracy, initial_auc, initial_ks, initial_recall = run_logistic_regression_minimal(
        X_train_full, y_train, X_test_full, y_test, random_state=random_state
    )
    print(f"Initial Accuracy: {initial_accuracy:.4f}")
    print(f"Initial Recall (Class 1): {initial_recall:.4f}") # Added Recall print
    print(f"Initial AUC: {initial_auc:.4f}")
    print(f"Initial KS: {initial_ks:.4f}")


    # Store models and their features/metrics at each step (including Recall)
    models_history = {'None': {'model': initial_model, 'features': list(feature_names), 'accuracy': initial_accuracy, 'auc': initial_auc, 'ks': initial_ks, 'recall': initial_recall}}
    results = {'Removed Group': ['None'], 'Accuracy': [initial_accuracy], 'AUC': [initial_auc], 'KS': [initial_ks], 'Recall': [initial_recall]} # Added Recall
    ...
    for i in range(len(sorted_groups) - 1):
        ...
        # Re-train model and get metrics (including recall)
        # Updated call to minimal runner
        model, accuracy, auc, ks, recall = run_logistic_regression_minimal(
            remaining_X_train, y_train, remaining_X_test, y_test, random_state=random_state
        )

        print(f"  Accuracy after removal: {accuracy:.4f}")
        print(f"  Recall (Class 1) after removal: {recall:.4f}") # Added Recall print
        ...
        results['Recall'].append(recall) # Added Recall
        # Store the model, its features, and metrics
        models_history[group_name_to_remove] = {'model': model, 'features': list(current_feature_names), 'accuracy': accuracy, 'auc': auc, 'ks': ks, 'recall': recall}


    results_df = pd.DataFrame(results)

    # Find the step with the highest RECALL
    best_step_index = results_df['Recall'].idxmax() # Changed from Accuracy to Recall
    best_removed_group = results_df.loc[best_step_index, 'Removed Group']
    best_recall = results_df.loc[best_step_index, 'Recall'] # Get best recall
    best_accuracy = results_df.loc[best_step_index, 'Accuracy'] # Get accuracy for the best recall model
    best_auc = results_df.loc[best_step_index, 'AUC'] # Get AUC for the best recall model
    best_ks = results_df.loc[best_step_index, 'KS'] # Get KS for the best recall model
    ...
    print(f"\nBest model (based on Recall for Class 1) found after removing group: '{best_removed_group}'") # Updated print
    print(f"Best Recall (Class 1): {best_recall:.4f}") # Updated print
    print(f"Accuracy for Best Recall Model: {best_accuracy:.4f}") # Updated print
    print(f"AUC for Best Recall Model: {best_auc:.4f}") # Updated print
    print(f"KS for Best Recall Model: {best_ks:.4f}") # Updated print
    print(f"Number of features in best model: {len(best_model_features)}")


    # Plotting (still plots Accuracy vs Removal Step, but highlight point is for best Accuracy *of the model selected by Recall*)
    fig, ax = plt.subplots(figsize=(14, 7))
    num_removed_steps = range(len(results_df))
    # Plot Accuracy
    ax.plot(num_removed_steps, results_df['Accuracy'], marker='o', linestyle='-', label='Accuracy')
    # Plot Recall on the same axis or a secondary axis if preferred
    # ax.plot(num_removed_steps, results_df['Recall'], marker='x', linestyle='--', label='Recall (Class 1)', color='orange') # Optional: Plot Recall too

    # Highlight the point corresponding to the best *recall* model's accuracy
    ax.scatter(best_step_index, best_accuracy, color='red', s=100, zorder=5, label=f'Best Recall Model (Accuracy: {best_accuracy:.4f}, Recall: {best_recall:.4f})') # Updated label
    ...
    ax.set_ylabel("Model Metric on Test Set") # Changed Y label
    ax.set_title("Sensitivity Analysis: Metrics vs. Feature Group Removal (Logistic Regression - Optimized for Recall)") # Updated title
    ax.legend()
...
graficos_dir = os.path.join(current_dir, 'graficos', 'modelo_regresion_recall')
...
print("\n--- Sensitivity Analysis for Model (All Features - Optimizing for Recall) ---") # Updated print
...
print("\nSensitivity Analysis Results (Optimized for Recall):") # Updated print
# Display relevant columns including Recall
print(results_df[['Removed Group', 'Accuracy', 'Recall', 'AUC', 'KS']]) # Added Recall to display

plot_path_sens = os.path.join(graficos_dir, 'sensitivity_analysis_model_recall_optimized.png') # Changed save path
...
# --- Metrics and Reports for Best Sensitivity Model (Selected by Recall) ---
if best_model_sens: # Check if a best model was found
    # Retrieve metrics directly from the results_df for the best step
    best_step_index = results_df['Recall'].idxmax() # Ensure we use recall index
    best_recall_sens = results_df.loc[best_step_index, 'Recall']
    ...
    print(f"\nStored the best model from sensitivity analysis (best_model_sens) based on Recall: {best_recall_sens:.4f}") # Updated print
    print(f"Features used in the best model ({len(best_features_sens)}): {best_features_sens}")

    # Calculate CM and Report String for the best model (using the test data associated with this best model)
    ...
    # Pass target_names for clearer report labels
    report_best_sens = classification_report(y_test_data_sens, y_pred_best_sens, target_names=['Non-Diabetic (0)', 'Diabetic (1)'], zero_division=0)

    # Print Metrics for the model selected based on best Recall
    print("\nMetrics for Best Model from Sensitivity Analysis (Selected by Recall) on Test Set:") # Updated print
    print(f"Recall (Class 1): {best_recall_sens:.4f}")
    ...
    best_recall_sens = None # Added recall default
    best_auc_sens = None
    best_ks_sens = None
...
# Model 1 (Full Features) - No change here needed
...
# Best Model from Sensitivity Analysis (Selected by Recall)
if best_model_sens: # Only plot if the best model exists
    total_support_best_sens = len(y_test_data_sens)
    # Changed save path and title
    report_table_path_best_sens = os.path.join(graficos_dir, 'classification_report_best_model_recall_sens.png')
    plot_classification_report_table(
        report_best_sens,
        best_accuracy_sens, # Accuracy of the best recall model
        best_auc_sens,      # AUC of the best recall model
        best_ks_sens,       # KS of the best recall model
        total_support_best_sens,
        f"Classification Report: Best Sensitivity Model (Optimized for Recall)", # Updated title
...
# --- Model 1 Plots --- (No change needed)
...
# --- Best Model Plots (Selected by Recall) ---
if best_model_sens:
    # Grouped Average Importance (Changed save path and title)
    importance_plot_path_best_avg = os.path.join(graficos_dir, 'feature_importance_best_model_recall_avg_table.png')
    plot_feature_importance(
        best_model_sens,
        best_features_sens,
        f"Avg Feature Importance: Best Recall Model (Grouped)", # Updated title
        importance_plot_path_best_avg
    )
    # Individual Importance (All) (Changed save path and title)
    importance_plot_path_best_ind = os.path.join(graficos_dir, 'feature_importance_best_model_recall_individual_table.png')
    plot_individual_feature_importance( # No top_n argument needed
        best_model_sens,
        best_features_sens,
        f"Individual Feature Importance: Best Recall Model", # Updated title
