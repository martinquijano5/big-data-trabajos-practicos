...
# Create new features using logarithmic transformation
# Note: We add a small constant (0.1) to avoid log(0) issues
iris_df['LogSepalLength'] = np.log(iris_df['SepalLengthCm'])
iris_df['LogSepalWidth'] = np.log(iris_df['SepalWidthCm'])
iris_df['LogPetalLength'] = np.log(iris_df['PetalLengthCm'])
iris_df['LogPetalWidth'] = np.log(iris_df['PetalWidthCm'])

# Print the head of the dataframe with new features
print("\nDataframe with logarithmic features:")
print(iris_df.head())
...
# Create a correlation matrix for only the logarithmic features
plt.figure(figsize=(10, 8))
# Select only the logarithmic features for correlation
log_features_df = iris_df[['LogSepalLength', 'LogSepalWidth', 'LogPetalLength', 'LogPetalWidth']]
# Calculate the correlation matrix
corr_matrix = log_features_df.corr()
# Create a heatmap of the correlation matrix
sns.heatmap(corr_matrix, annot=True, cmap='coolwarm', fmt='.2f', linewidths=0.5)
plt.title('Correlation Matrix of Logarithmic Features')
plt.tight_layout()

# Create a boxplot for only the new logarithmic features
plt.figure(figsize=(15, 10))

# Boxplots for new logarithmic features
plt.subplot(2, 2, 1)
sns.boxplot(x='Species', y='LogSepalLength', data=iris_df)
plt.title('Boxplot of Log Sepal Length by Species')

plt.subplot(2, 2, 2)
sns.boxplot(x='Species', y='LogSepalWidth', data=iris_df)
plt.title('Boxplot of Log Sepal Width by Species')

plt.subplot(2, 2, 3)
sns.boxplot(x='Species', y='LogPetalLength', data=iris_df)
plt.title('Boxplot of Log Petal Length by Species')

plt.subplot(2, 2, 4)
sns.boxplot(x='Species', y='LogPetalWidth', data=iris_df)
plt.title('Boxplot of Log Petal Width by Species')
...
# Prepare the data with logarithmic features
X = iris_df[['LogSepalLength', 'LogSepalWidth', 'LogPetalLength', 'LogPetalWidth']]  # Using only the logarithmic features
y = iris_df['Species']  # Target variable

# For comparison, also prepare the original features
X_original = iris_df.drop(['Species', 'Id', 'LogSepalLength', 'LogSepalWidth', 'LogPetalLength', 'LogPetalWidth'], axis=1)

# Scale both feature sets with separate scalers
scaler_log = StandardScaler().fit(X)
...
# Find the optimal k value using cross-validation for logarithmic features
k_range = range(1, 51)
cv_scores = []
cv_scores_original = []
...
for k in k_range:
    ...
    # Cross-validation with logarithmic features
...
plt.plot(k_range, cv_scores, label='Logarithmic Features')
...
# Find the best k value for logarithmic features
best_k = 0
best_accuracy = 0
...
print(f"\nBest k value for logarithmic features: {best_k} with accuracy: {best_accuracy:.4f}")
print(f"Best k value for original features: {best_k_original} with accuracy: {best_accuracy_original:.4f}")

# Train the model with the best k value on the logarithmic features
best_knn = KNeighborsClassifier(n_neighbors=best_k)
best_knn.fit(X_scaled, y)

# Create a confusion matrix using cross-validation predictions for logarithmic features
y_pred = cross_val_predict(best_knn, X_scaled, y, cv=10)

print("\nCross-Validation Classification Report (Logarithmic Features):")
print(classification_report(y, y_pred))

print("\nCross-Validation Confusion Matrix (Logarithmic Features):")
...
plt.ylabel('Actual')
plt.title(f'Confusion Matrix - Logarithmic Features (k={best_k})')
...
# Create a decision boundary visualization with the logarithmic features
# We'll use the two most important logarithmic features for visualization
plt.figure(figsize=(10, 8))

# Select the two most important logarithmic features based on boxplot separation
feature1 = 'LogPetalLength'
feature2 = 'LogPetalWidth'
X_viz = iris_df[[feature1, feature2]]

# Create a mesh grid for the selected logarithmic features
...
plt.xlabel(feature1)
plt.ylabel(feature2)
plt.title(f'Decision Boundaries with Selected Logarithmic Features (k={best_k})')
...
# Prepare the data with logarithmic features
X_log = iris_df[['LogSepalLength', 'LogSepalWidth', 'LogPetalLength', 'LogPetalWidth']]  # Using only the logarithmic features
y_log = iris_df['Species']  # Target variable

# Scale the features - use the same X_log for fitting and transforming
X_log_scaled = scaler_log.transform(X_log)

# Create and train the logistic regression model
log_reg = LogisticRegression(max_iter=1000, random_state=42)

# Perform cross-validation with logarithmic features
...
print(f"\nLogistic Regression Cross-Validation Accuracy (Logarithmic Features): {log_cv_accuracy:.4f}")
...
print("\nLogistic Regression Classification Report (Logarithmic Features):")
...
print("\nLogistic Regression Confusion Matrix (Logarithmic Features):")
...
plt.xlabel('Predicted')
plt.ylabel('Actual')
plt.title('Logistic Regression Confusion Matrix - Logarithmic Features')
...
# Prepare the data with logarithmic features
X_kmeans = iris_df[['LogSepalLength', 'LogSepalWidth', 'LogPetalLength', 'LogPetalWidth']]  # Using only the logarithmic features
y_true = iris_df['Species']  # True labels (only for evaluation)

# Scale the features
X_kmeans_scaled = scaler_log.transform(X_kmeans)
...
# Evaluate clustering performance with logarithmic features
rand_score = adjusted_rand_score(y_true, mapped_labels)
print(f"\nAdjusted Rand Score (Logarithmic Features): {rand_score:.4f}")

# Also perform K-means with original features for comparison
X_kmeans_original = iris_df.drop(['Species', 'Id', 'LogSepalLength', 'LogSepalWidth', 'LogPetalLength', 'LogPetalWidth'], axis=1)
...
print("\nK-means Clustering Confusion Matrix (Logarithmic Features):")
...
plt.title('K-means Confusion Matrix - Logarithmic Features')
...
# Visualize the clusters in 2D using the two most important logarithmic features
plt.figure(figsize=(16, 8))

# Select the two most important logarithmic features for visualization
feature1 = 'LogPetalLength'
feature2 = 'LogPetalWidth'
X_viz = iris_df[[feature1, feature2]]
...
plt.xlabel(feature1)
plt.ylabel(feature2)
plt.title('K-means Clusters (Selected Logarithmic Features)')
...
plt.xlabel(feature1)
plt.ylabel(feature2)
plt.title('True Species (Selected Logarithmic Features)')
...
plt.suptitle('K-means Clusters vs. True Species (Selected Logarithmic Features)', fontsize=16)
plt.tight_layout(rect=[0, 0, 1, 0.95])  # Adjust layout to make room for suptitle

# Add a summary section comparing all models with both feature sets
print("\n--- Model Performance Summary ---")
print(f"KNN (Logarithmic Features): {best_accuracy:.4f}")
print(f"KNN (Original Features): {best_accuracy_original:.4f}")
print(f"Logistic Regression (Logarithmic Features): {log_cv_accuracy:.4f}")
print(f"Logistic Regression (Original Features): {log_cv_accuracy_original:.4f}")
print(f"K-means (Logarithmic Features): {rand_score:.4f}")
...
log_scores = [best_accuracy, log_cv_accuracy, rand_score]
orig_scores = [best_accuracy_original, log_cv_accuracy_original, rand_score_original]

x = np.arange(len(models))
width = 0.35

plt.bar(x - width/2, log_scores, width, label='Logarithmic Features')
