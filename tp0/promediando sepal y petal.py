...
# Create new features by averaging width and height
iris_df['AvgSepalSize'] = (iris_df['SepalLengthCm'] + iris_df['SepalWidthCm']) / 2
iris_df['AvgPetalSize'] = (iris_df['PetalLengthCm'] + iris_df['PetalWidthCm']) / 2
...
avg_features_df = iris_df[['AvgSepalSize', 'AvgPetalSize']]
# Calculate the correlation matrix
corr_matrix = avg_features_df.corr()
# Create a heatmap of the correlation matrix
sns.heatmap(corr_matrix, annot=True, cmap='coolwarm', fmt='.2f', linewidths=0.5)
plt.title('Correlation Matrix of Averaged Features (Sepal/Petal Size)')
plt.tight_layout()
...
plt.subplot(3, 2, 5)
sns.boxplot(x='Species', y='AvgSepalSize', data=iris_df)
plt.title('Boxplot of Average Sepal Size by Species')

plt.subplot(3, 2, 6)
sns.boxplot(x='Species', y='AvgPetalSize', data=iris_df)
plt.title('Boxplot of Average Petal Size by Species')
...
X = iris_df[['AvgSepalSize', 'AvgPetalSize']]  # Using only the averaged features
y = iris_df['Species']  # Target variable

# For comparison, also prepare the original features
X_original = iris_df.drop(['Species', 'Id', 'AvgSepalSize', 'AvgPetalSize'], axis=1)
...
plt.plot(k_range, cv_scores, label='Averaged Features')
...
print(f"\nBest k value for averaged features: {best_k} with accuracy: {best_accuracy:.4f}")
...
print("\nCross-Validation Classification Report (Averaged Features):")
print(classification_report(y, y_pred))

print("\nCross-Validation Confusion Matrix (Averaged Features):")
...
plt.ylabel('Actual')
plt.title(f'Confusion Matrix - Averaged Features (k={best_k})')
...
x_min, x_max = X['AvgSepalSize'].min() - 0.5, X['AvgSepalSize'].max() + 0.5
y_min, y_max = X['AvgPetalSize'].min() - 0.5, X['AvgPetalSize'].max() + 0.5
...
scatter = plt.scatter(X['AvgSepalSize'], X['AvgPetalSize'], c=y_numeric, 
           edgecolor='k', s=50, cmap='viridis')

# Highlight misclassified points with a red circle
misclassified = y_pred != y
plt.scatter(X['AvgSepalSize'][misclassified], X['AvgPetalSize'][misclassified], 
            s=80, facecolors='none', edgecolors='r', linewidths=2)

plt.xlabel('Average Sepal Size')
plt.ylabel('Average Petal Size')
plt.title(f'Decision Boundaries with Averaged Features (k={best_k})')
...
# Prepare the data with averaged features
X_log = iris_df[['AvgSepalSize', 'AvgPetalSize']]  # Using only the averaged features
...
print(f"\nLogistic Regression Cross-Validation Accuracy (Averaged Features): {log_cv_accuracy:.4f}")
...
print("\nLogistic Regression Classification Report (Averaged Features):")
...
print("\nLogistic Regression Confusion Matrix (Averaged Features):")
...
plt.xlabel('Predicted')
plt.ylabel('Actual')
plt.title('Logistic Regression Confusion Matrix - Averaged Features')
...
# Prepare the data with averaged features
X_kmeans = iris_df[['AvgSepalSize', 'AvgPetalSize']]  # Using only the averaged features
...
print(f"\nAdjusted Rand Score (Averaged Features): {rand_score:.4f}")

# Also perform K-means with original features for comparison
X_kmeans_original = iris_df.drop(['Species', 'Id', 'AvgSepalSize', 'AvgPetalSize'], axis=1)
...
print("\nK-means Clustering Confusion Matrix (Averaged Features):")
...
plt.title('K-means Confusion Matrix - Averaged Features')
...
plt.subplot(1, 2, 1)
scatter_kmeans = plt.scatter(X_kmeans['AvgSepalSize'], X_kmeans['AvgPetalSize'], c=cluster_labels, 
                            cmap='viridis', edgecolor='k', s=50)
plt.xlabel('Average Sepal Size')
plt.ylabel('Average Petal Size')
plt.title('K-means Clusters (Averaged Features)')
...
scatter_true = plt.scatter(X_kmeans['AvgSepalSize'], X_kmeans['AvgPetalSize'], c=y_numeric, 
                          cmap='viridis', edgecolor='k', s=50)
plt.xlabel('Average Sepal Size')
plt.ylabel('Average Petal Size')
plt.title('True Species (Averaged Features)')
...
plt.suptitle('K-means Clusters vs. True Species (Averaged Features)', fontsize=16)
plt.tight_layout(rect=[0, 0, 1, 0.95])  # Adjust layout to make room for suptitle

# Add a summary section comparing all models with both feature sets
print("\n--- Model Performance Summary ---")
print(f"KNN (Averaged Features): {best_accuracy:.4f}")
print(f"KNN (Original Features): {best_accuracy_original:.4f}")
print(f"Logistic Regression (Averaged Features): {log_cv_accuracy:.4f}")
print(f"Logistic Regression (Original Features): {log_cv_accuracy_original:.4f}")
print(f"K-means (Averaged Features): {rand_score:.4f}")
...
plt.bar(x - width/2, avg_scores, width, label='Averaged Features')
...
#al usar los promedios da mejor accuracy en KNN, misma accuracy en regresion logistica pero empeora en kmeans