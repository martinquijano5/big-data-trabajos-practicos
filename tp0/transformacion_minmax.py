...
# Create MinMax scaled features
# First, extract the original features
X_original = iris_df[['SepalLengthCm', 'SepalWidthCm', 'PetalLengthCm', 'PetalWidthCm']].values

# Apply MinMax scaling
minmax_scaler = MinMaxScaler()
X_minmax = minmax_scaler.fit_transform(X_original)

# Create a new DataFrame with MinMax scaled features
minmax_df = pd.DataFrame(X_minmax, columns=['SepalLength_MinMax', 'SepalWidth_MinMax', 
                                           'PetalLength_MinMax', 'PetalWidth_MinMax'])

# Add the species column
minmax_df['Species'] = iris_df['Species'].values
minmax_df['Id'] = iris_df['Id'].values

# Print the head of the dataframe with MinMax scaled features
print("\nDataframe with MinMax scaled features:")
print(minmax_df.head())

# Count the number of each species
sums = {}
for species, count in minmax_df['Species'].value_counts().items():
    sums[species] = count
print("sums: " + str(sums))

# Create a correlation matrix for the MinMax scaled features
plt.figure(figsize=(10, 8))
# Calculate the correlation matrix
corr_matrix = minmax_df[['SepalLength_MinMax', 'SepalWidth_MinMax', 
                         'PetalLength_MinMax', 'PetalWidth_MinMax']].corr()
# Create a heatmap of the correlation matrix
sns.heatmap(corr_matrix, annot=True, cmap='coolwarm', fmt='.2f', linewidths=0.5)
plt.title('Correlation Matrix of MinMax Scaled Features')
plt.tight_layout()

# Create boxplots to visualize the distribution of MinMax scaled features by species
plt.figure(figsize=(14, 10))
for i, feature in enumerate(['SepalLength_MinMax', 'SepalWidth_MinMax', 
                            'PetalLength_MinMax', 'PetalWidth_MinMax']):
    plt.subplot(2, 2, i+1)
    sns.boxplot(x='Species', y=feature, data=minmax_df)
...
# Prepare the data with MinMax scaled features
X = minmax_df[['SepalLength_MinMax', 'SepalWidth_MinMax', 
              'PetalLength_MinMax', 'PetalWidth_MinMax']]  # Using MinMax scaled features
y = minmax_df['Species']  # Target variable

# For comparison, also prepare the original features
X_original_df = iris_df[['SepalLengthCm', 'SepalWidthCm', 'PetalLengthCm', 'PetalWidthCm']]

# Scale the original features with StandardScaler for fair comparison
scaler_orig = StandardScaler().fit(X_original_df)
X_original_scaled = scaler_orig.transform(X_original_df)

# Find the optimal k value using cross-validation for MinMax scaled features
k_range = range(1, 51)
cv_scores = []
cv_scores_original = []
...
for k in k_range:
    ...
    # Cross-validation with MinMax scaled features
...
plt.plot(k_range, cv_scores, label='MinMax Scaled Features')
plt.plot(k_range, cv_scores_original, label='Original Features (StandardScaler)')
...
# Find the best k value for MinMax scaled features
best_k = 0
best_accuracy = 0
...
print(f"\nBest k value for MinMax scaled features: {best_k} with accuracy: {best_accuracy:.4f}")
print(f"Best k value for original features: {best_k_original} with accuracy: {best_accuracy_original:.4f}")

# Train the model with the best k value on the MinMax scaled features
best_knn = KNeighborsClassifier(n_neighbors=best_k)
best_knn.fit(X, y)

# Create a confusion matrix using cross-validation predictions for MinMax scaled features
y_pred = cross_val_predict(best_knn, X, y, cv=10)

print("\nCross-Validation Classification Report (MinMax Scaled Features):")
print(classification_report(y, y_pred))

print("\nCross-Validation Confusion Matrix (MinMax Scaled Features):")
...
plt.ylabel('Actual')
plt.title(f'Confusion Matrix - MinMax Scaled Features (k={best_k})')
...
# Create a decision boundary visualization with the MinMax scaled features
# We'll use the two most important features for visualization
plt.figure(figsize=(10, 8))

# Select the two most important features based on domain knowledge
feature1 = 'PetalLength_MinMax'
feature2 = 'PetalWidth_MinMax'
X_viz = minmax_df[[feature1, feature2]]

# Create a mesh grid for the selected features
x_min, x_max = X_viz[feature1].min() - 0.1, X_viz[feature1].max() + 0.1
y_min, y_max = X_viz[feature2].min() - 0.1, X_viz[feature2].max() + 0.1
xx, yy = np.meshgrid(np.arange(x_min, x_max, 0.01),
                     np.arange(y_min, y_max, 0.01))

# Create a KNN model with just these two features
knn_viz = KNeighborsClassifier(n_neighbors=best_k)
knn_viz.fit(X_viz, y)

# Get predictions
mesh_points = np.c_[xx.ravel(), yy.ravel()]
Z = knn_viz.predict(mesh_points)
...
y_pred_viz = cross_val_predict(knn_viz, X_viz, y, cv=10)
...
plt.xlabel(feature1)
plt.ylabel(feature2)
plt.title(f'Decision Boundaries with MinMax Scaled Features (k={best_k})')
...
# Prepare the data with MinMax scaled features
X_log = minmax_df[['SepalLength_MinMax', 'SepalWidth_MinMax', 
                  'PetalLength_MinMax', 'PetalWidth_MinMax']]
y_log = minmax_df['Species']  # Target variable

# Create and train the logistic regression model
log_reg = LogisticRegression(max_iter=1000, random_state=42)

# Perform cross-validation with MinMax scaled features
log_cv_scores = cross_val_score(log_reg, X_log, y_log, cv=10, scoring='accuracy')
log_cv_accuracy = log_cv_scores.mean()

# Also perform cross-validation with original features for comparison
log_cv_scores_original = cross_val_score(log_reg, X_original_scaled, y_log, cv=10, scoring='accuracy')
log_cv_accuracy_original = log_cv_scores_original.mean()

print(f"\nLogistic Regression Cross-Validation Accuracy (MinMax Scaled Features): {log_cv_accuracy:.4f}")
...
log_y_pred = cross_val_predict(log_reg, X_log, y_log, cv=10)
log_y_pred_original = cross_val_predict(log_reg, X_original_scaled, y_log, cv=10)
...
print("\nLogistic Regression Classification Report (MinMax Scaled Features):")
...
print("\nLogistic Regression Confusion Matrix (MinMax Scaled Features):")
...
plt.xlabel('Predicted')
plt.ylabel('Actual')
plt.title('Logistic Regression Confusion Matrix - MinMax Scaled Features')
...
# Prepare the data with MinMax scaled features
X_kmeans = minmax_df[['SepalLength_MinMax', 'SepalWidth_MinMax', 
                     'PetalLength_MinMax', 'PetalWidth_MinMax']]
y_true = minmax_df['Species']  # True labels (only for evaluation)
...
# Evaluate clustering performance with MinMax scaled features
rand_score = adjusted_rand_score(y_true, mapped_labels)
print(f"\nAdjusted Rand Score (MinMax Scaled Features): {rand_score:.4f}")

# Also perform K-means with original features for comparison
X_kmeans_original = X_original_df
...
print("\nK-means Clustering Confusion Matrix (MinMax Scaled Features):")
...
plt.title('K-means Confusion Matrix - MinMax Scaled Features')
...
# Visualize the clusters in 2D using the two most important features
...
plt.xlabel(feature1)
plt.ylabel(feature2)
plt.title('K-means Clusters (MinMax Scaled Features)')
...
plt.xlabel(feature1)
plt.ylabel(feature2)
plt.title('True Species (MinMax Scaled Features)')
...
plt.suptitle('K-means Clusters vs. True Species (MinMax Scaled Features)', fontsize=16)
plt.tight_layout(rect=[0, 0, 1, 0.95])  # Adjust layout to make room for suptitle

# Add a summary section comparing all models with both feature sets
print("\n--- Model Performance Summary ---")
print(f"KNN (MinMax Scaled Features): {best_accuracy:.4f}")
print(f"KNN (Original Features): {best_accuracy_original:.4f}")
print(f"Logistic Regression (MinMax Scaled Features): {log_cv_accuracy:.4f}")
print(f"Logistic Regression (Original Features): {log_cv_accuracy_original:.4f}")
print(f"K-means (MinMax Scaled Features): {rand_score:.4f}")
...
minmax_scores = [best_accuracy, log_cv_accuracy, rand_score]
orig_scores = [best_accuracy_original, log_cv_accuracy_original, rand_score_original]

x = np.arange(len(models))
width = 0.35

plt.bar(x - width/2, minmax_scores, width, label='MinMax Scaled Features')
...
for i, v in enumerate(minmax_scores):
