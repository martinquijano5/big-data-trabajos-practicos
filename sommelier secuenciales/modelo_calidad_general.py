...
def plot_decision_boundary(X_plot, y_plot, model_to_predict_with, scaler_original, feature_names_plot, directory, data_split_name, model_name_prefix, pca_transformer=None):
    ...
    if y_max <= y_min + 1e-7: # Epsilon for linspace distinctiveness
        ...
        min_fallback_total_height = 0.005 # desired minimum total y-axis span
...
def plot_model_metrics(model, X_test_scaled, y_test, X_train_scaled, y_train, X_full_scaled, y_full, feature_names, directory, model_name_prefix):
    ...
    quality_labels_for_report = [f'Quality {i}' for i in range(3, 10)]
    report_dict_test = classification_report(y_test, y_pred_test_for_report, target_names=quality_labels_for_report, output_dict=True, zero_division=0)
    ...
    print(f"\nClassification Report for {model_name_prefix} (Test Set):\n", classification_report(y_test, y_pred_test_for_report, target_names=quality_labels_for_report, zero_division=0))
    ...
    report_dict_full = classification_report(y_full, y_pred_full_for_report, target_names=quality_labels_for_report, output_dict=True, zero_division=0)
    ...
    print(f"\nClassification Report for {model_name_prefix} (Full Data):\n", classification_report(y_full, y_pred_full_for_report, target_names=quality_labels_for_report, zero_division=0))
...
parent_dir = os.path.join(current_dir, 'graficos', f'modelo_calidad_general_{datetime.datetime.now().strftime("%Y%m%d_%H%M%S")}') # Changed here
os.makedirs(parent_dir, exist_ok=True)


#leer los datos
# For the general model, we need combined_data
white_wine_data_path = os.path.join(current_dir, 'generate_color_csv', 'white_wine_data_scaled.csv')
red_wine_data_path = os.path.join(current_dir, 'generate_color_csv', 'red_wine_data_scaled.csv')

# It seems the original script used placeholder names for the actual scaled data.
# I will assume the files are named 'white_wine_data_scaled.csv' and 'red_wine_data_scaled.csv'
# in the 'sommelier secuenciales/generate_color_csv/' directory as per previous discussions.
# If these are not the correct filenames, they will need to be adjusted.
try:
    white_wine_data = pd.read_csv(white_wine_data_path, sep=',')
    print(f"Successfully loaded white wine data from: {white_wine_data_path}")
except FileNotFoundError:
    print(f"Error: Could not find white wine data at {white_wine_data_path}. Please ensure the file exists.")
    white_wine_data = pd.DataFrame() # Create empty df to avoid further errors if file not found
...
try:
    ...
    red_wine_data = pd.DataFrame() # Create empty df

if not white_wine_data.empty and not red_wine_data.empty:
    combined_data = pd.concat([white_wine_data, red_wine_data], axis=0, ignore_index=True)
    print("\nCombined Data Head:")
    print(combined_data.head())
    combined_data.name = 'combined_data'
elif not white_wine_data.empty:
    print("Warning: Red wine data not loaded. Using only white wine data for combined_data.")
    combined_data = white_wine_data.copy()
    combined_data.name = 'combined_data_white_only'
elif not red_wine_data.empty:
    print("Warning: White wine data not loaded. Using only red wine data for combined_data.")
    combined_data = red_wine_data.copy()
    combined_data.name = 'combined_data_red_only'
else:
    print("Error: Neither white nor red wine data could be loaded. Cannot proceed with general model.")
    combined_data = pd.DataFrame() # Empty DataFrame


if not combined_data.empty:
    #MODELO CALIDAD GENERAL
    #directorio modelo general
    general_dir = os.path.join(parent_dir, 'modelo_general')
    os.makedirs(general_dir, exist_ok=True)

    #directorio modelo simple
    simple_dir = os.path.join(general_dir, 'modelo_simple')
    ...
    intermediate_dir = os.path.join(general_dir, 'modelo_intermedio')
    ...
    complete_dir = os.path.join(general_dir, 'modelo_completo')
    ...
    # Assuming 'alcohol' and 'pH' are desired for the general simple model
    # and are present in the scaled CSVs.
    # Adjust if these are not the correct column names in your scaled data.
    feature_columns_simple_general = ['alcohol', 'density'] 
    target_column_general = 'quality'

    # Check if required columns exist before proceeding
    if all(col in combined_data.columns for col in feature_columns_simple_general) and target_column_general in combined_data.columns:
        scatterplot_vs_quality(combined_data, feature_columns_simple_general[0], feature_columns_simple_general[1], simple_dir)

        print("\n--- General Simple model: Combined Neural Network and Logistic Regression Sensibility Analysis (1 Hidden Layer) ---")
        neural_network_neurons_sensibility_analysis(combined_data, feature_columns_simple_general, target_column_general, simple_nn_base_regression_dir)

        print("\n--- General Simple model: Neural Network Sensibility Analysis (2 Hidden Layers) ---")
        neural_network_two_hidden_layers_sensibility_analysis(combined_data, feature_columns_simple_general, target_column_general, simple_nn_dir_2_layers)
    else:
        print(f"Skipping General Simple Model: Not all required columns ({feature_columns_simple_general + [target_column_general]}) found in combined_data.")


    #modelo intermedio 5 variables
    feature_columns_intermediate_general = ['alcohol', 'density', 'chlorides', 'pH', 'volatile acidity']
    # target_column_general is already defined

    if all(col in combined_data.columns for col in feature_columns_intermediate_general) and target_column_general in combined_data.columns:
        plot_pca(combined_data, feature_columns_intermediate_general, target_column_general, intermediate_dir)

        print("\n--- General Intermediate model: Combined Neural Network and Logistic Regression Sensibility Analysis (1 Hidden Layer) ---")
        neural_network_neurons_sensibility_analysis(combined_data, feature_columns_intermediate_general, target_column_general, intermediate_nn_base_regression_dir)

        print("\n--- General Intermediate model: Neural Network Sensibility Analysis (2 Hidden Layers) ---")
        neural_network_two_hidden_layers_sensibility_analysis(combined_data, feature_columns_intermediate_general, target_column_general, intermediate_nn_dir_2_layers)
    else:
        print(f"Skipping General Intermediate Model: Not all required columns ({feature_columns_intermediate_general + [target_column_general]}) found in combined_data.")

    #modelo completo todas variables
    # Assuming all these features are present in your scaled CSVs.
    feature_columns_complete_general = ['fixed acidity', 'volatile acidity', 'citric acid', 'residual sugar', 'chlorides', 'free sulfur dioxide', 'total sulfur dioxide', 'density', 'pH', 'sulphates', 'alcohol']
    # target_column_general is already defined

    if all(col in combined_data.columns for col in feature_columns_complete_general) and target_column_general in combined_data.columns:
        plot_pca(combined_data, feature_columns_complete_general, target_column_general, complete_dir)

        print("\n--- General Complete model: Combined Neural Network and Logistic Regression Sensibility Analysis (1 Hidden Layer) ---")
        neural_network_neurons_sensibility_analysis(combined_data, feature_columns_complete_general, target_column_general, complete_nn_base_regression_dir)

        print("\n--- General Complete model: Neural Network Sensibility Analysis (2 Hidden Layers) ---")
        neural_network_two_hidden_layers_sensibility_analysis(combined_data, feature_columns_complete_general, target_column_general, complete_nn_dir_2_layers)
    else:
        print(f"Skipping General Complete Model: Not all required columns ({feature_columns_complete_general + [target_column_general]}) found in combined_data.")
else:
    print("Skipping General Model processing as combined_data is empty.")
