...
parent_dir = os.path.join(current_dir, 'graficos', f'modelo_calidad_blanco_{datetime.datetime.now().strftime("%Y%m%d_%H%M%S")}') # Changed here
os.makedirs(parent_dir, exist_ok=True)


#leer los datos
# For the white wine model, we only need white_wine_data
...
try:
    ...
    print("\nWhite Wine Data Head:")
    print(white_wine_data.head())
    white_wine_data.name = 'white_wine_data_scaled' # Giving a more descriptive name
except FileNotFoundError:
    print(f"Error: Could not find white wine data at {white_wine_data_path}. Please ensure the file exists.")
    print("Cannot proceed with White Wine Model without data.")
    white_wine_data = pd.DataFrame() # Assign empty DataFrame


if not white_wine_data.empty:
    #MODELO CALIDAD PARA VINO BLANCO
    #directorio modelo vino blanco
    white_wine_dir = os.path.join(parent_dir, 'modelo_blanco') # This is the main folder for this script's outputs
    os.makedirs(white_wine_dir, exist_ok=True)

    #directorio modelo simple
    simple_dir = os.path.join(white_wine_dir, 'modelo_simple')
    ...
    intermediate_dir = os.path.join(white_wine_dir, 'modelo_intermedio')
    ...
    complete_dir = os.path.join(white_wine_dir, 'modelo_completo')
    ...
    # Using features as per original script for white wine simple model
    feature_columns_simple_white = ['alcohol', 'volatile acidity'] 
    target_column_white = 'quality'

    if all(col in white_wine_data.columns for col in feature_columns_simple_white) and target_column_white in white_wine_data.columns:
        scatterplot_vs_quality(white_wine_data, feature_columns_simple_white[0], feature_columns_simple_white[1], simple_dir)

        print("\n--- White Wine Simple model: Combined Neural Network and Logistic Regression Sensibility Analysis (1 Hidden Layer) ---")
        neural_network_neurons_sensibility_analysis(white_wine_data, feature_columns_simple_white, target_column_white, simple_nn_base_regression_dir)

        print("\n--- White Wine Simple model: Neural Network Sensibility Analysis (2 Hidden Layers) ---")
        neural_network_two_hidden_layers_sensibility_analysis(white_wine_data, feature_columns_simple_white, target_column_white, simple_nn_dir_2_layers)
    else:
        print(f"Skipping White Wine Simple Model: Not all required columns ({feature_columns_simple_white + [target_column_white]}) found in white_wine_data.")


    #modelo intermedio 5 variables
    # Using features as per original script for white wine intermediate model
    feature_columns_intermediate_white = ['alcohol', 'density', 'chlorides', 'volatile acidity', 'residual sugar']
    # target_column_white is already defined

    if all(col in white_wine_data.columns for col in feature_columns_intermediate_white) and target_column_white in white_wine_data.columns:
        plot_pca(white_wine_data, feature_columns_intermediate_white, target_column_white, intermediate_dir)

        print("\n--- White Wine Intermediate model: Combined Neural Network and Logistic Regression Sensibility Analysis (1 Hidden Layer) ---")
        neural_network_neurons_sensibility_analysis(white_wine_data, feature_columns_intermediate_white, target_column_white, intermediate_nn_base_regression_dir)

        print("\n--- White Wine Intermediate model: Neural Network Sensibility Analysis (2 Hidden Layers) ---")
        neural_network_two_hidden_layers_sensibility_analysis(white_wine_data, feature_columns_intermediate_white, target_column_white, intermediate_nn_dir_2_layers)
    else:
        print(f"Skipping White Wine Intermediate Model: Not all required columns ({feature_columns_intermediate_white + [target_column_white]}) found in white_wine_data.")

    #modelo completo todas variables
    feature_columns_complete_white = ['fixed acidity', 'volatile acidity', 'citric acid', 'residual sugar', 'chlorides', 'free sulfur dioxide', 'total sulfur dioxide', 'density', 'pH', 'sulphates', 'alcohol']
    # target_column_white is already defined

    if all(col in white_wine_data.columns for col in feature_columns_complete_white) and target_column_white in white_wine_data.columns:
        plot_pca(white_wine_data, feature_columns_complete_white, target_column_white, complete_dir)

        print("\n--- White Wine Complete model: Combined Neural Network and Logistic Regression Sensibility Analysis (1 Hidden Layer) ---")
        neural_network_neurons_sensibility_analysis(white_wine_data, feature_columns_complete_white, target_column_white, complete_nn_base_regression_dir)

        print("\n--- White Wine Complete model: Neural Network Sensibility Analysis (2 Hidden Layers) ---")
        neural_network_two_hidden_layers_sensibility_analysis(white_wine_data, feature_columns_complete_white, target_column_white, complete_nn_dir_2_layers)
    else:
        print(f"Skipping White Wine Complete Model: Not all required columns ({feature_columns_complete_white + [target_column_white]}) found in white_wine_data.")
else:
    print("Skipping White Wine Model processing as white_wine_data is empty.")
